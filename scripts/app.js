// ===== CONFIGURAÇÕES =====
        ...
        let usuarioAtual = '';
        let motorEstado = 'off';      // 'on', 'off', 'auto'
        let autoMode = false;         // false = manual, true = automático
        ...
        // ===== CONTROLE DO MOTOR (LÓGICA CORRIGIDA) =====
        function controlarMotor(comando) {
            ...
            if (comando === 'auto') {
                ...
                if (database) {
                    database.ref('comandos/motor').set({ estado: 'auto', timestamp: Date.now() });
                }
            ...
            // Se chegou aqui, é comando manual (on/off) e autoMode = false
            if (comando === 'on') {
                // Liga motor
                ...
                // Enviar comando para Firebase
                if (database) {
                    database.ref('comandos/motor').set({ estado: 'on', timestamp: Date.now() });
                }
                
                // Timer de segurança - desliga após 2 minutos
                setTimeout(() => {
                ...
                // Enviar comando para Firebase
                if (database) {
                    database.ref('comandos/motor').set({ estado: 'off', timestamp: Date.now() });
                }
            }
        }
        ...
        function toggleAutoMode() {
            ...
            if (autoMode) {
                ...
                // Desabilitar botão auto (já está ativo)
                document.querySelector('.btn-control.auto').classList.remove('active');
                
                // Atualizar status do motor (se estava em auto, mantém estado atual)
        ...
        // ===== ATUALIZAR DADOS DO FIREBASE (COM LÓGICA DO MODO) =====
        function atualizarDadosFirebase(dados) {
            ...
            if (dados.solo !== undefined && dados.solo !== null) {
                ...
                if (!isNaN(solo)) {
                    ...
                    // CONTROLE AUTOMÁTICO (SÓ SE ESTIVER EM MODO AUTO)
                    if (autoMode) {
            ...
            // Atualizar botões conforme o modo
            if (autoMode) {
                document.querySelector('.btn-control.on').disabled = true;
                document.querySelector('.btn-control.off').disabled = true;
                document.querySelector('.btn-control.on').style.opacity = '0.5';
        ...
        function exportarDados() {
            ...
            database.ref('historico').limitToLast(100).once('value', s => {
                ...
                a.download = `terraflow_${new Date().toISOString().split('T')[0]}.csv`;
                a.click();
